...
(function($, window, document, undefined) {
    ...
    var defaults = {
		...
		controlsCallback: function (item) { return item.controls || '' ? item.controls : ''; },
        listNodeName: 'ol',
        itemNodeName: 'li',
        handleNodeName: 'div',
        ...
        listRenderer: function(children, options) {
            var html = '<' + options.listNodeName + ' class="' + options.listClass + '" data-group="' + options.group + '">';
            html += children;
            html += '</' + options.listNodeName + '>';

            return html;
        },
        itemRenderer: function(item_attrs, content, children, controls, options, item) {
			var item_attrs_string = $.map(item_attrs, function(value, key) {
				if (key !== 'data-controls')
				return ' ' + key + '="' + value + '"';
            }).join(' ');
			...
			if (controls) {
				html += '<' + options.handleNodeName + ' class="' + options.controlsClass + '">';
				html += controls;
				html += '</' + options.handleNodeName + '>';
			}
    ...
    Plugin.prototype = {
        ...
        add: function (item)
        {
            ...
            var emptyPlaceholderSelector = '.' + this.options.emptyClass;
            if($(this.el).children(emptyPlaceholderSelector).length > 0) {
                var list = $(document.createElement(this.options.listNodeName)).addClass(this.options.listClass);
                $(this.el).children(emptyPlaceholderSelector).replaceWith(list);
            }
        ...
        removeAll: function(callback){
            ...
            function remove(){
				...
				node.replaceWith('<div class="' + opts.emptyClass + '"/>');
        ...
        _buildItem: function(item, options) {
			...
			var controls = options.controlsCallback(item);
            var children = this._buildList(item.children, options);
            var html = $(options.itemRenderer(item_attrs, content, children, controls, options, item));
        ...
        serialize: function() {
            var data, list = this, step = function(level) {
                ...
                items.each(function() {
                    var li = $(this),
                        item = $.extend({content: li.text()}, li.data()),
        ...
        dragStop: function(e) {
            ...
            if (typeof continueExecution !== 'undefined' && continueExecution === false) {
				...
				if (this.el.find('.dd-empty').length) {
					this.el.find('.dd-empty').replaceWith($(document.createElement(this.options.listNodeName)).addClass(this.options.listClass));
				}
